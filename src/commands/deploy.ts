...
export function registerDeployCommands(program: Command): void {
    ...
    deploy
        ...
        .option('--types <types>', 'Filter by types (comma-separated: PUBLISH,DEPLOY,ROLLBACK,UNDEPLOY)')
        ...
        .action(async (opts) => {
            try {
                ...
                const data = await apiRequest<PageResponse<DeploymentSummary>>(
                    'GET',
                    'deployments',
                    {
                        apiKey: globalOpts.apiKey,
    ...
    deploy
        ...
        .action(async (opts) => {
            try {
                const globalOpts = program.opts();
                const data = await apiRequest<DeploymentDetail>(
                    'GET',
                    `deployments/${opts.id}`,
                    {
                        apiKey: globalOpts.apiKey,
    ...
    deploy
        ...
        .action(async () => {
            try {
                ...
                const data = await apiRequest<DeploymentStatus[]>(
                    'GET',
